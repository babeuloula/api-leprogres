...
class Parser
{
    ...
    private function oneContent(object $result): array
    {
        ...
        $content['image']['url'] = (true === isset($result->Details->Images[0]))
            ? $result->Details->Images[0]->Url
            : $result->ImageUrl;
        $content['image']['caption'] = (true === isset($result->Details->Images[0]))
            ? $result->Details->Images[0]->Caption
            : $result->ImageCaption;
        ...
        if (false === empty($result->TagSections[0]->Tags)) {
    ...
    {
        $date = DateTime::createFromFormat($in, $datetime);
